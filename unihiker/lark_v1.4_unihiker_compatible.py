...
def draw_ui():
    ...
    if current_state == STATE_READY:
        ...
        radius_outer = 60 + int(pulse_size)  # Reduced from 80 to 60 for better fit
    ...
    # Draw circle outline by drawing two circles
    gui.draw_circle(x=120, y=120, r=60, color=circle_color)  # Reduced from 80 to 60
    gui.draw_circle(x=120, y=120, r=58, color=DARK_BLUE)  # Reduced from 78 to 58
    
    # Draw LARK text in center - removed size parameter
    gui.draw_text(text="LARK", x=120, y=120, color=WHITE)
    ...
    gui.draw_text(text=status_texts[current_state], x=120, y=180, color=WHITE)
    ...
    if len(message) > 20:  # Reduced from 30 to 20
        message = message[:17] + "..."  # Reduced from 27 to 17
